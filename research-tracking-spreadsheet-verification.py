...
def check_spreadsheet_title(word, sheet_num):
   ...
   # filtered_df = sheet_data[sheet_data.iloc[:, 3].str.contains(word, case=False).fillna(False)]
    sheet_data["matching"] = sheet_data.iloc[:, 3].map(lambda x: fuzz.ratio(str(x), word, processor=utils.default_process))
...
def compute_count(topic, initials, card_num):
    """
    Computes the number of cards in the file by searching for initials. This relies on having initials for each debater stored in a separate file.
    """
    topic_cols = {'SEPTOBER': 3,'NOCEMBER': 5, 'JANUARY':7, 'FEBRUARY': 9, 'APRIL' : 11, 'JUNE': 13}
    gc = pygsheets.authorize(service_file='creds.json')
    sh = gc.open('Card Counts 2023-24')
    wks = sh.sheet1
